...
export async function getMeetings(req, res) {
  ...
  try {
    const response = await ntlmPost({
      url: ewsUrl, username: user, password, domain, workstation: "", rejectUnauthorized: false,
      body:    buildSOAP(startISO, endISO),
    ...
    res.json({ meetings: parseXML(response.body) });
  } catch (err) {
...
export async function getEmails(req, res) {
  ...
  try {
    ...
    res.json({ emails: parseEmails(response.body) });
  } catch (err) {
    const msg = err.message || "";
    if (msg.includes("ENOTFOUND") || msg.includes("ETIMEDOUT") || msg.includes("ECONNREFUSED"))
      return res.status(502).json({ error: `Cannot reach Exchange server at ${ewsUrl}. Check the URL.` });
...
export async function getEmailBody(req, res) {
  ...
  let domain = "", user = username;
  if (username.includes("\\")) [domain, user] = username.split("\\");
  const ntlmOpts = { url: ewsUrl, username: user, password, domain, workstation: "", rejectUnauthorized: false };
...
export async function sendEmail(req, res) {
  ...
  try {
    ...
    if (response.body.includes("soap:Fault")) {
      ...
      return res.status(500).json({ error: `Exchange error: ${fault ? fault[1] : "Unknown SOAP fault"}` });
    }
  ...
  try {
    ...
    if (response.body.includes("ResponseCode")) {
      ...
      if (code && code[1] !== "NoError") {
        ...
        return res.status(500).json({ error: `Exchange: ${msg2 ? msg2[1] : code[1]}` });
      }
    }
